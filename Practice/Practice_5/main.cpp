...
tuple<vector<Coord>, double, vector<string>> dijkstra(Graph graph, Coord start, Coord end) {
    ...
    // shortest_paths[start] никогда не вызовется, добавлен для логичности содержания словаря
    ...
    while (!priority_queue.empty()) {
        ...
        // т.к. прямой маршрут оказался меньше других по длине
        ...
        if (it != graph.end()) {
            for (auto& neighbor_pair : it->second) {
                ...
                // Расстояние до соседа с учетом пройденного пути
    ...
    while (true) {
        ...
        // Если текущий узел — стартовый, то выходим из цикла
        if (current_node == start) {
            // Цепочка [end; start] (оба включительно) уже построена
            break;
        }
        // Заменяем текущий узел на узел-предшественник
...
tuple<map<string, Coord>, vector<EdgeItem>> read_graphml(const string& file_path) {
    ...
    for (tinyxml2::XMLElement* edge = graph->FirstChildElement("edge"); edge; edge = edge->NextSiblingElement("edge")) {
        ...
        for (tinyxml2::XMLElement* data = edge->FirstChildElement("data"); data; data = data->NextSiblingElement("data")) {
            ...
            if (strcmp(key, "d14") == 0 && data->GetText()) { // street name
...
auto measure_execution_time(Func&& func, Args&&... args) {
    ...
    return make_pair(result, duration_sec.count());
}
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("city_road_network.graphml");
    //  print_edges(edges);
    ...
    // Определение start_index и end_index, т.к. известны заранее
    int start_index = 4383;
    int end_index = 25678;
    ...
    if (start_index == -1 || end_index == -1) {
        ...
        std::tie(path, distance, street_names) = result;
        // Выводим время работы алгоритма Дейкстры
        std::cout << "ВРЕМЯ РАБОТЫ АЛГОРИТМА ДЕЙКСТРЫ: " << round_to_n_digits(execution_time, 2) << " сек" << std::endl;

        if (path.empty()) {
            std::cout << "Путь не найден" << std::endl;
        }
        else {
            std::cout << "Найден путь длиной " << round_to_n_digits(distance, 1) << " км" << std::endl;
